...
public final class ResponseTypeIntrospector {
  ...
  public Optional<String> extractDataRefName(Method method) {
    ...
    if (log.isDebugEnabled()) {
      log.debug("Introspected method [{}]: wrapper [{}], data [{}]",
              method.toGenericString(), raw.getSimpleName(), ref.orElse("<none>"));
    }
  ...
  private ResolvableType nextLayer(ResolvableType current, Class<?> raw) {
    return switch (raw) {
      ...
      case Class<?> c when CompletionStage.class.isAssignableFrom(c)
              || Future.class.isAssignableFrom(c) -> current.getGeneric(0);
      case Class<?> c when DeferredResult.class.isAssignableFrom(c)
              || WebAsyncTask.class.isAssignableFrom(c) -> current.getGeneric(0);
