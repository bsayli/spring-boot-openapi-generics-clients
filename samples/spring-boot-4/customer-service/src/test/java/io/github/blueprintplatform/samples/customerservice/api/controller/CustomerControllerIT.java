...
import io.github.blueprintplatform.samples.customerservice.api.error.ApiRequestExceptionHandler;
import io.github.blueprintplatform.samples.customerservice.api.error.ApplicationExceptionHandler;
import io.github.blueprintplatform.samples.customerservice.common.api.sort.SortField;
import io.github.blueprintplatform.samples.customerservice.config.JacksonConfig;
import io.github.blueprintplatform.samples.customerservice.service.CustomerService;
import io.github.blueprintplatform.samples.customerservice.testconfig.TestControllerMocksConfig;
...
class CustomerControllerIT {
  ...
  @Test
  @DisplayName("POST /customers -> 201 Created, Location header ve ServiceResponse(data, meta)")
  ...
  @Test
  @DisplayName("POST /customers -> 400 validation error (MethodArgumentNotValid)")
  ...
  @Test
  @DisplayName("POST /customers -> 400 invalid JSON (HttpMessageNotReadable)")
  ...
  @Test
  @DisplayName("GET /customers/{id} -> 200 OK (one customer)")
  void getCustomer_ok200() throws Exception {
    ...
    mvc.perform(get("/customers/{id}", 1).header(API_VERSION_HEADER, API_VERSION))
  ...
  @Test
  @DisplayName("GET /customers/{id} -> 404 NOT_FOUND (NoSuchElementException)")
  void getCustomer_notFound404() throws Exception {
    ...
    mvc.perform(get("/customers/{id}", 99).header(API_VERSION_HEADER, API_VERSION))
        ...
        .andExpect(jsonPath("$.instance").value("/customers/99"))
  ...
  @Test
  @DisplayName("GET /customers/{id} -> 404 NOT_FOUND fallback message")
  void getCustomer_notFound404_fallbackMessage() throws Exception {
    ...
    mvc.perform(get("/customers/{id}", 77).header(API_VERSION_HEADER, API_VERSION))
        .andExpect(status().isNotFound())
        .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_PROBLEM_JSON))
        .andExpect(jsonPath("$.type").value("urn:customer-service:problem:not-found"))
        .andExpect(jsonPath("$.title").value("Resource not found"))
        .andExpect(jsonPath("$.status").value(404))
        .andExpect(jsonPath("$.detail").value("Requested resource was not found."))
        .andExpect(jsonPath("$.instance").value("/customers/77"))
        .andExpect(jsonPath("$.errorCode").value("NOT_FOUND"))
        .andExpect(jsonPath("$.extensions.errors[0].code").value("NOT_FOUND"))
  ...
  @Test
  @DisplayName("GET /customers/{id} -> 400 BAD_REQUEST (type mismatch)")
  void getCustomer_typeMismatch400() throws Exception {
    mvc.perform(get("/customers/{id}", "abc").header(API_VERSION_HEADER, API_VERSION))
        .andExpect(status().isBadRequest())
        .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_PROBLEM_JSON))
        .andExpect(jsonPath("$.type").value("urn:customer-service:problem:bad-request"))
        .andExpect(jsonPath("$.title").value("Bad request"))
        ...
        .andExpect(jsonPath("$.instance").value("/customers/abc"))
        .andExpect(jsonPath("$.errorCode").value("BAD_REQUEST"))
        .andExpect(jsonPath("$.extensions.errors[0].code").value("BAD_REQUEST"))
        .andExpect(
  ...
  @Test
  @DisplayName("GET /customers/{id} -> 400 validation error (@Min violation)")
  void getCustomer_constraintViolation_min() throws Exception {
    mvc.perform(get("/customers/{id}", 0).header(API_VERSION_HEADER, API_VERSION))
        .andExpect(status().isBadRequest())
        .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_PROBLEM_JSON))
        .andExpect(jsonPath("$.type").value("urn:customer-service:problem:validation-failed"))
        .andExpect(jsonPath("$.title").value("Validation failed"))
        .andExpect(jsonPath("$.status").value(400))
        .andExpect(jsonPath("$.detail").value("One or more fields are invalid."))
        .andExpect(jsonPath("$.instance").value("/customers/0"))
        .andExpect(jsonPath("$.errorCode").value("VALIDATION_FAILED"))
        .andExpect(jsonPath("$.extensions.errors[0].code").value("VALIDATION_FAILED"));
  }

  @Test
  @DisplayName("GET /customers/{id} -> 500 Internal Server Error (RFC 9457 ProblemDetail)")
  ...
  @Test
  @DisplayName("GET /customers -> 200 OK, Page<CustomerDto> + default meta.sort")
  void getCustomers_list200_defaultSort() throws Exception {
    ...
    mvc.perform(get("/customers").header(API_VERSION_HEADER, API_VERSION))
        .andExpect(status().isOk())
        .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
  ...
  @Test
  @DisplayName("GET /customers -> 200 OK, custom sort metadata")
  ...
  @Test
  @DisplayName("GET /customers -> 400 BAD_REQUEST (sortBy type mismatch)")
  void getCustomers_sortBy_typeMismatch400() throws Exception {
    mvc.perform(get("/customers").header(API_VERSION_HEADER, API_VERSION).param("sortBy", "foo"))
        .andExpect(status().isBadRequest())
        .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_PROBLEM_JSON))
        .andExpect(jsonPath("$.type").value("urn:customer-service:problem:bad-request"))
        .andExpect(jsonPath("$.title").value("Bad request"))
  ...
  @Test
  @DisplayName("GET /customers -> 400 validation error (page @Min)")
  void getCustomers_page_validation_min() throws Exception {
    mvc.perform(get("/customers").header(API_VERSION_HEADER, API_VERSION).param("page", "-1"))
        .andExpect(status().isBadRequest())
        .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_PROBLEM_JSON))
        .andExpect(jsonPath("$.type").value("urn:customer-service:problem:validation-failed"))
        .andExpect(jsonPath("$.title").value("Validation failed"))
  ...
  @Test
  @DisplayName("GET /customers -> 400 validation error (size @Min)")
  void getCustomers_size_validation_min() throws Exception {
    mvc.perform(get("/customers").header(API_VERSION_HEADER, API_VERSION).param("size", "0"))
        .andExpect(status().isBadRequest())
        .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_PROBLEM_JSON))
        .andExpect(jsonPath("$.type").value("urn:customer-service:problem:validation-failed"))
        .andExpect(jsonPath("$.title").value("Validation failed"))
  ...
  @Test
  @DisplayName("GET /customers -> 400 validation error (size @Max)")
  void getCustomers_size_validation_max() throws Exception {
    mvc.perform(get("/customers").header(API_VERSION_HEADER, API_VERSION).param("size", "11"))
        .andExpect(status().isBadRequest())
        .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_PROBLEM_JSON))
        .andExpect(jsonPath("$.type").value("urn:customer-service:problem:validation-failed"))
        .andExpect(jsonPath("$.title").value("Validation failed"))
  ...
  @Test
  @DisplayName("PUT /customers/{id} -> 200 OK (update)")
  ...
  @Test
  @DisplayName("PUT /customers/{id} -> 400 validation error (MethodArgumentNotValid)")
  ...
  @Test
  @DisplayName("DELETE /customers/{id} -> 204 No Content")
  void deleteCustomer_noContent204() throws Exception {
    doNothing().when(customerService).deleteCustomer(1);

    mvc.perform(delete("/customers/{id}", 1).header(API_VERSION_HEADER, API_VERSION))
        .andExpect(status().isNoContent())
        .andExpect(content().string(""));
  }

  @Test
  @DisplayName("DELETE /customers -> 405 method not allowed")
  void collectionDelete_methodNotAllowed405() throws Exception {
    mvc.perform(delete("/customers").header(API_VERSION_HEADER, API_VERSION))
